...
from .utils import Mono, PadCrop_Normalized_T, PhaseFlipper, Stereo, VolumeNorm

AUDIO_KEYS = ("flac", "wav", "mp3", "m4a", "ogg", "opus", ".aiff")
...
def get_audio_filenames(
    ...
    keywords=None,
    exts=[".wav", ".mp3", ".flac", ".ogg", ".aif", ".opus", ".aiff"],
):
...
def get_latent_filenames(paths: list, extensions=["npy"]):  # directories in which to search
...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if max_files is not None:
            ...
            print(f"Using only first {max_files} files")
