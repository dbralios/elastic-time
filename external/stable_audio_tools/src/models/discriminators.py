...
class MultiScaleDiscriminator(nn.Module):
    ...
    def __init__(self, in_channels: int, n_scales: int, **conv_kwargs) -> None:
        ...
        for _ in range(n_scales):
            layers.append(
                SharedDiscriminatorConvNet(in_channels, nn.Conv1d, **conv_kwargs)
            )
...
class MultiPeriodDiscriminator(nn.Module):
    ...
    def __init__(
        self, in_channels: int, periods: tp.Sequence[int], **conv_kwargs
    ) -> None:
        super().__init__()
        layers = []
        ...
        for _ in periods:
            layers.append(
                SharedDiscriminatorConvNet(in_channels, nn.Conv2d, **conv_kwargs)
            )
...
class OobleckDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        multi_period_discriminator = MultiPeriodDiscriminator(
            in_channels=in_channels, periods=[2, 3, 5, 7, 11]
        )
