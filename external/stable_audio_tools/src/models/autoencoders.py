...
from ..inference.utils import prepare_audio  # Okay
from .blocks import SnakeBeta  # Okay
from .bottleneck import Bottleneck, DiscreteBottleneck  # Okay

from .factory import (
    create_pretransform_from_config,
    create_bottleneck_from_config,
)  # Okay
from .pretransforms import Pretransform  # Okay
...
class AudioAutoencoder(nn.Module):
    ...
    def forward(self, audio):
        x_pre = self.preprocess_audio_for_encoder(audio, in_sr=self.sample_rate)
        encoded = self.encode_audio(x_pre)
        decoded = self.decode_audio(encoded)
        return decoded[..., : audio.shape[-1]]
