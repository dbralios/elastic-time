"""
Modified Muon optimizer implementation.
https://github.com/Wayfarer-Labs/owl-vaes/blob/a3917af560423c46eac3e55998f51a961f22a961/owl_vaes/muon.py#L35
"""
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(
        self, params, lr=0.02, weight_decay=0.01, momentum=0.95, nesterov=True, ns_steps=5, rank=None, world_size=None
    ):
...
class CombinedOptimizer(Optimizer):
    def __init__(self, params, rank=0, world_size=1, **kwargs):
        ...
        # muon_parameters = [
        #     p for n, p in model.named_parameters() if not any(key in n for key in adamw_keys) and p.ndim >= 2
        # ]
        params = list(params)
        adamw_parameters = [p for p in params if p.ndim < 2]
        muon_parameters = [p for p in params if p.ndim >= 2]

        print(
            f"Initialized CombinedOptimizer with {len(adamw_parameters)} AdamW params and {len(muon_parameters)} Muon params."
        )
        ...
        self.muon = Muon(
            muon_parameters, lr=kwargs.get("lr"), momentum=kwargs.get("momentum"), rank=rank, world_size=world_size
        )
