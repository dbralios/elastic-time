...
from ._device import get_world_size_and_rank
...
class WandBLogger(MetricLoggerInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.watch_cfg = kwargs.pop("watch", None)
    ...
    def watch(self, model: torch.nn.Module) -> None:
        """Watches the model for logging gradients and parameters.

        Args:
            model (torch.nn.Module): model to watch
        """
        if self.watch_cfg is None:
            log.info("No watch config provided. Skipping model.watch()")
            return

        if self._wandb.run:
            self._wandb.watch(model, **self.watch_cfg)
...
class CometLogger(MetricLoggerInterface):
    ...
    def log_config(self, config: DictConfig) -> None:
        if self.experiment is not None:
            ...
            output_config_fname = save_config(config)
            try:
                self.experiment.log_asset(output_config_fname, file_name=output_config_fname.name)
            except Exception as e:
