...
def get_constant_schedule_with_warmup(
    ...
    **kwargs,
    ...
    """
    Create a learning rate schedule that linearly increases the learning rate from
    0.0 to lr over ``num_warmup_steps``, then keeps it constant at lr.
...
def get_constant_schedule_with_warmup_and_delay(
    optimizer: torch.optim.Optimizer,
    num_warmup_steps: int,
    delay_steps: int,
    last_epoch: int = -1,
    **kwargs,
    ...
    def lr_lambda(current_step: int) -> float:
        ...
        if current_step < delay_steps:
            return 0.0
        if current_step < num_warmup_steps + delay_steps:
            return current_step / max(1, num_warmup_steps)
...
def get_lr(optimizer: torch.optim.Optimizer) -> float:
    ...
    Args:
        optimizer (torch.optim.Optimizer): A general
